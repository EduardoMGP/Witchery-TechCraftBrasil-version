...
public class TransformWolfman {
   ...
   public void syncModelWith(EntityLivingBase entity, boolean frontface) {
      if(this.proxyEntity == null) {
         this.proxyEntity = new EntityWolfman(entity.worldObj);
      ...
      this.proxyEntity.setCurrentItemOrArmor(0, entity.getHeldItem());
      this.proxyEntity.setEating(entity.isEating());
      this.proxyEntity.setSitting(entity.isRiding());
      if(entity instanceof EntityPlayer) {
         EntityPlayer player = (EntityPlayer)entity;
         this.proxyEntity.setItemInUse(player.getItemInUse() == null?null:player.getItemInUse().copy(), player.getItemInUseCount());
      }
   ...
   public void render(World worldObj, EntityLivingBase entity, double x, double y, double z, RendererLivingEntity renderer, float partialTicks, boolean frontface) {
      ...
      double d3 = 0.0D - (double)this.proxyEntity.yOffset;
      if(this.proxyEntity.isSneaking() && !(entity instanceof EntityPlayerSP)) {
         d3 -= 0.125D;
      }

      if(this.proxyEntity.isRiding()) {
         Entity ridden = this.proxyEntity.ridingEntity;
         d3 += ridden.getMountedYOffset();
      }
