...
public class TransformBat {
   ...
   public void syncModelWith(EntityLivingBase entity, boolean frontface) {
      if(this.proxyEntity == null) {
         this.proxyEntity = new EntityBat(entity.worldObj);
      ...
      this.proxyEntity.setIsBatHanging(false);
   ...
   public void render(World worldObj, EntityLivingBase entity, double x, double y, double z, RendererLivingEntity renderer, float partialTicks, boolean frontface) {
      ...
      Vec3 vec = this.proxyEntity.getLookVec();
      vec.rotateAroundY(90.0F);
      GL11.glPushMatrix();
      GL11.glScalef(0.8F, 0.8F, 0.8F);
      this.proxyEntity.ticksExisted += 2;
      this.proxyRenderer.doRender(this.proxyEntity, x + vec.xCoord * 0.75D, y + d3 - 0.6D, z + vec.zCoord * 0.75D, frontface?0.0F:f1, partialTicks);
      this.proxyEntity.ticksExisted += 5;
