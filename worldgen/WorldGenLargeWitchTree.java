...
public class WorldGenLargeWitchTree extends WorldGenerator {
      ...
      for(aint1[par5] = aint[par5]; j1 <= i1; ++j1) {
         ...
         while(k1 <= i1) {
            ...
            if(d0 > (double)(par4 * par4)) {
               ...
               if(l1 != Blocks.air && l1 != Witchery.Blocks.LEAVES) {
                  ++k1;
               } else {
                  this.setBlockAndNotifyAdequately(this.worldObj, aint1[0], aint1[1], aint1[2], blockID, meta);
                  ++k1;
               }
            }
         }
   ...
   void placeBlockLine(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger, Block blockId, int meta) {
      ...
      if(aint2[b1] != 0) {
         ...
         for(int k = aint2[b1] + b4; j != k; j += b4) {
            ...
            this.setBlockAndNotifyAdequately(this.worldObj, aint3[0], aint3[1], aint3[2], blockId, meta);
         }
      }
   ...
   int checkBlockLine(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger) {
      ...
      if(aint2[b1] == 0) {
         ...
         for(j = aint2[b1] + b4; i != j; i += b4) {
            ...
            if(k != Blocks.air && k != Witchery.Blocks.LEAVES) {
               break;
            }
         }
   ...
   boolean validTreeLocation() {
      ...
      boolean isValidSoil = i != null && i.canSustainPlant(this.worldObj, this.basePos[0], this.basePos[1] - 1, this.basePos[2], ForgeDirection.UP, (IPlantable)Witchery.Blocks.SAPLING);
      if(!isValidSoil) {
         return false;
      } else {
